...
p1 <- ggplot(data_p1, aes(x = year, y = value, color = Gas)) +
...
saveRDS(p1, file = "./Steve/figures/lineplot_stations.rds")
...
# 3. Generate Figure
p2 <- ggplot(hourly_variation, aes(x = hour, y = Concentration, color = Gas)) +
...
png(filename = "./Steve/figures/radarplot_top3.png")
...
png(filename = "./Steve/figures/radarplot_top6.png")
radarchart(radar_final, 
           axistype = 1,
           pcol = gas_colors, 
           pfcol = gas_fill, 
...
p6 <- ggplot(global_hourly, aes(x = hour, y = Concentration, color = Gas)) +
    geom_line(linewidth = 1.5) +
    geom_point(size = 2) +
...
# Stations: if we look at the station with the worst pollution value for each pollutant
# in 2018, we end up with these stations:
# Pza. Fernández Ladred for PM10, 
# Casa de Campo for O3
# Escuelas Aguirre for NO2
...
#png(filename = "./Steve/figures/radarplot_top3_interactive.png")
radarchart(radar_final, 
           axistype = 1,
           pcol = colors_border, 
           pfcol = colors_in, 
           ...
           cglwd = 0.8,
           vlcex = 0.8,
           title = "Top 3 Pollutants Patterns Across Stations")
legend(
    x = 1.1, y = 1.2,
    legend = rownames(radar_df), 
    bty = "n",              
...
# 2. Prepare data for the radar plot
radar_prep_2018 <- madrid_complete %>%
